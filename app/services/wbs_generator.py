...
def _money(val):
    """Format as money if float, otherwise blank."""
    try:
        return float(val)
    except Exception:
        return 0.0
...
# ---------------- template write (robust header finder) ----------------
def _find_wbs_header(ws: Worksheet) -> Tuple[int, Dict[str, int]]:
    ...
    Works even if headers are merged or slightly renamed (e.g., 'Totals').
    """
    # Aliases we accept for each logical column (lowercased, no newlines)
    req_aliases = {
        "ssn":        ["ssn"],
    ...
    opt_aliases = {
        ...
        "total_plain":["total", "totals"],  # far-right pink/orange column
    }
    ...
    # Score each row by how many required aliases appear somewhere on it.
    best_row, best_score, best_map = None, -1, None
    for r in range(1, ws.max_row + 1):
        row_vals = [norm_cell(ws.cell(r, c).value) for c in range(1, ws.max_column + 1)]

        # quick skip if row is mostly empty
        if sum(1 for v in row_vals if v) < 3:
            continue

        # For each required field, check if any alias is present in ANY cell of the row
        score = 0
        for key, aliases in req_aliases.items():
            if any(any(a in v for a in aliases) for v in row_vals):
                score += 1

        if score > best_score:
            # Build a column map for this row (first matching alias wins)
            col_map = {}
    ...
    if best_row is None or best_map is None:
        raise HTTPException(
            422,
            detail="WEEKLY header not found. Expected columns like 'Employee Name', 'A01', 'A02', 'A03', etc.",
        )
...
def write_into_wbs_template(template_bytes: bytes, weekly: pd.DataFrame) -> bytes:
    """
    Writes data ONLY to the 'WEEKLY' sheet so the file matches the WBS layout.
    Populates SSN, Employee Name, Status, Type, Pay Rate, Dept, A01, A02, A03.
    Also populates $ columns if present; otherwise fills far-right 'Total/Totals'.
    """
